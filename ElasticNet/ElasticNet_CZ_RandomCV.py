...
def ElasticNet_KFold_RandomCV_AllSubsets(Subjects_Data_Mat_Path, Subjects_Score, SampleInfo, Fold_Quantity, CVRepeatTimes, Alpha_Range, L1_ratio_Range, ResultantFolder, Max_Queued, QueueOptions, Parallel_Quantity):
        ...
        for j in np.arange(Times_SampleResample):
            if not os.path.exists(ResultantFolder_I + '/Prediction_' + str(j) + '.mat'):
                ...
                Configuration_Mat = {'Subjects_Data_Mat_Path': Subjects_Data_Mat_Path, 'Subjects_Score': Subjects_Score, 'SampleSize': SampleSize_Array[i], \
                    'Fold_Quantity': Fold_Quantity, 'CVRepeatTimes': CVRepeatTimes, 'Sample_Index': j, 'Selected_IDs': Selected_IDs, 'Alpha_Range': Alpha_Range, 'L1_ratio_Range': L1_ratio_Range, 'ResultantFolder_I': ResultantFolder_I, 'Parallel_Quantity': Parallel_Quantity};
                ...
                system_cmd = 'python3 -c ' + '\'import sys;\
                    ...
                    from ElasticNet_CZ_RandomCV import ElasticNet_KFold_RandomCV_OneSubset;\
                    import os;\
                    import scipy.io as sio;\
                    ...
                    ElasticNet_KFold_RandomCV_OneSubset(Subjects_Data_Mat_Path[0], Subjects_Score[0], Selected_IDs[0], Sample_Index[0][0], Fold_Quantity[0][0], CVRepeatTimes[0][0], Alpha_Range[0], L1_ratio_Range[0], ResultantFolder_I[0], Parallel_Quantity[0][0])\' ';
...
def ElasticNet_KFold_RandomCV_OneSubset(Subjects_Data_Mat_Path, Subjects_Score, SelectedIDs, SampleIndex, Fold_Quantity, CVRepeatTimes, Alpha_Range, L1_ratio_Range, ResultantFolder, Parallel_Quantity):
    ...
    for i in np.arange(CVRepeatTimes):
        ...
        Corr_I, MAE_I = ElasticNet_KFold_RandomCV(Data_Selected, Scores_Selected, Fold_Quantity, Alpha_Range, L1_ratio_Range, CVRepeatTimes, ResultantFolder_SampleI_TimeI, Parallel_Quantity)
