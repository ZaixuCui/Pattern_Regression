...
def Ridge_KFold_RandomCV_AllSubsets(Subjects_Data_Mat_Path, Subjects_Score, SampleInfo, Fold_Quantity, CVRepeatTimes, Alpha_Range, ResultantFolder, Max_Queued, QueueOptions, Parallel_Quantity):
        ...
        for j in np.arange(Times_SampleResample):
            if not os.path.exists(ResultantFolder_I + '/Prediction_' + str(j) + '.mat'):
                ...
                system_cmd = 'python3 -c ' + '\'import sys;\
                    sys.path.append("/lustre/gaolab/cuizaixu/Utilities_Zaixu/Utilities_Regression/Ridge");\
                    from Ridge_CZ_RandomCV import Ridge_KFold_RandomCV_OneSubset;\
                    import os;\
                    import scipy.io as sio;\
                    ...
                    Ridge_KFold_RandomCV_OneSubset(Subjects_Data_Mat_Path[0], Subjects_Score[0], Selected_IDs[0], Sample_Index[0][0], Fold_Quantity[0][0], CVRepeatTimes[0][0], Alpha_Range[0], ResultantFolder_I[0], Parallel_Quantity[0][0])\' ';
...
def Ridge_KFold_RandomCV_OneSubset(Subjects_Data_Mat_Path, Subjects_Score, SelectedIDs, SampleIndex, Fold_Quantity, CVRepeatTimes, Alpha_Range, ResultantFolder, Parallel_Quantity):
    ...
    for i in np.arange(CVRepeatTimes):
        ...
        Corr_I, MAE_I = Ridge_KFold_RandomCV(Data_Selected, Scores_Selected, Fold_Quantity, Alpha_Range, CVRepeatTimes, ResultantFolder_SampleI_TimeI, Parallel_Quantity)
...
def Ridge_KFold_RandomCV(Subjects_Data, Subjects_Score, Fold_Quantity, Alpha_Range, CVRepeatTimes_ForInner, ResultantFolder, Parallel_Quantity):
    ...
    for j in np.arange(Fold_Quantity):
        ...
        Optimal_Alpha = Ridge_OptimalAlpha_KFold(Subjects_Data_train, Subjects_Score_train, Fold_Quantity, Alpha_Range, CVRepeatTimes_ForInner, ResultantFolder, Parallel_Quantity)
